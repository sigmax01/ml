...
???+ question "为什么MLP被认为是Translationally Equivariant的"

    “平移不变形”指的是, **~~无论特征出现在图像的哪个位置, 网络都能在那个相应的位置产出类似的响应, 并且模型无需为不同位置的同类特征单独学习不同的参数~~, 无论输入token的位置如何变化, 对每个token的处理方式都相同.**

    对于MSA, 进行平移后, 不同token可能相对位置会发生变化, 导致对于每个token来说, 最终的注意力得分发生变化. 所以对每个token来说, 其注意力处理方式是会发生变化的.

    对于MLP而言, 不管如何进行平移, 对于每个token的处理方式都是不会发生变化的, 都是经过相同的非线性变换.
