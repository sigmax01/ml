...
- $\bm{U}$: 这是一个$n\times m$的正交矩阵(没有降维), $n\times k$的正交矩阵(有降维)
- $\bm{V^T}$: 这是一个$m\times m$的正交矩阵$V$的转置(没有降维), $k\times m$的正交矩阵(有降维)
- $\bm{\Lambda}$: 这一一个$m\times m$的对角矩阵, 其中包含奇异值(没有降维), $k\times k$的对角矩阵(有降维)
