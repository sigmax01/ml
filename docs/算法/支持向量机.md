...
我们来看一种比较常用的核函数: RBF核函数, 这种表达式的函数为$K(\bm{x}, \bm{y})=exp(-\frac{||\bm{x}-\bm{y}||}{2\sigma^2})$. 这个函数有一个超参数, Hyper Parameter, 即$\sigma$, 结合我们在[软边际和硬边际](#软边际和硬边际)中讲到的另一个超参数$C$. 我们在训练中常常会调节这两个参数, 尝试将它们组合, 然后评估结果, 如:
